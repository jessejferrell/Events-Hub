...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/vendor-registrations", requireAuth, async (req, res) => {
    try {
      console.log("Creating vendor registration with data:", req.body);
      
      // First, verify the user has a vendor profile
      const vendorProfile = await storage.getVendorProfile(req.user.id);
      if (!vendorProfile) {
        return res.status(400).json({ message: "Vendor profile not found. Please create a profile first." });
      }
      ...
      // Verify the vendor spot exists
      const vendorSpot = await storage.getProduct(req.body.vendorSpotId);
      if (!vendorSpot || vendorSpot.type !== 'vendor_spot') {
        return res.status(404).json({ message: "Vendor spot not found" });
      }
      
      // Prepare data for insertion with proper references
      const registrationData = {
        vendorProfileId: vendorProfile.id,
        vendorSpotId: req.body.vendorSpotId,
        eventId: req.body.eventId,
        status: "pending",
        productsDescription: req.body.productsDescription,
        specialRequests: req.body.specialRequirements,
        notes: `Electricity: ${req.body.needsElectricity ? 'Yes' : 'No'}, Water: ${req.body.needsWater ? 'Yes' : 'No'}, Preferred Location: ${req.body.preferredLocation || 'None specified'}`,
        metadata: {
          needsElectricity: req.body.needsElectricity,
          needsWater: req.body.needsWater,
          preferredLocation: req.body.preferredLocation
        }
      };
      
      // Validate using schema after preparing data
      const validatedData = insertVendorRegistrationSchema.parse(registrationData);
      ...
      await storage.recordAnalyticEvent({
        ...
        metric: "vendor_registration",
        value: 1,
        dimension: "user_id", 
        dimensionValue: req.user.id.toString(),
      });
      
      console.log("Vendor registration created successfully:", registration);
      ...
      if (error instanceof z.ZodError) {
        return res.status(400).json({ 
          ...
          errors: error.errors,
          details: "Schema validation failed on the prepared data"
        });
      }
